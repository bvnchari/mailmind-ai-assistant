...
with tab1:
    ...
    with col_a:
        tone = st.selectbox("Select Tone", ["Formal", "Friendly", "Assertive", "Urgent"])
        recipient = st.text_input("Recipient Name", placeholder="e.g. Hiring Manager")
    with col_b:
        email_to = st.text_input("Recipient Email (for Direct Send)")
        subject = st.text_input("Email Subject", placeholder="e.g. Application for Role")
        
    prompt = st.text_area("Email Goal (Describe what you want to say)", height=150)

    if st.button("Generate ✨"):
        if prompt and recipient:
            st.session_state.thread_id = str(uuid.uuid4())
            final_state = mailmind_app.invoke(
                {"prompt": prompt, "tone": tone, "recipient": recipient, "is_mock": is_mock}, 
                get_config()
            )
    ...
    if current_state.next:
        ...
        edited = st.text_area("Final Polish:", value=draft, height=300)
            ...
            if st.button("🚀 Approve & Send Now"):
                if not email_to or not subject:
                    ...
                    if is_mock:
                        ...
                        success, msg = send_real_email(email_to, subject, edited)
        ...
        # --- NEW: Download Section for Tab 1 ---
        ...
        with d_col2:
            ...
            pdf.cell(0, 10, f"To: {recipient}", ln=True)
        ...
        if st.session_state.bulk_drafts:
            ...
            for idx, item in enumerate(st.session_state.bulk_drafts):
                with st.expander(f"Review: {item['name']} - {item['subject']}"):
                    new_subj = st.text_input("Subject:", value=item['subject'], key=f"subj_{idx}")
                    new_txt = st.text_area("Body:", value=item['draft'], key=f"body_{idx}", height=200)
                    st.session_state.bulk_drafts[idx]['subject'] = new_subj
                    st.session_state.bulk_drafts[idx]['draft'] = new_txt
            ...
            with b2:
                ...
                for item in st.session_state.bulk_drafts:
                    ...
                    doc.add_paragraph(f"Subject: {item['subject']}", style='Intense Quote')
                    doc.add_paragraph(item['draft'])
                    doc.add_page_break()
                bio_word = io.BytesIO()
                doc.save(bio_word)
                st.download_button("📥 Download Word (.docx)", data=bio_word.getvalue(), file_name="Bulk_Emails.docx")
            ...
            with b3:
                ...
                st.download_button("📥 Download PDF (.pdf)", data=pdf_output, file_name="Bulk_Emails.pdf")
